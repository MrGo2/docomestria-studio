...
### Added
- "Ver documento reconstruido" button in the studio panel header. Opens a
  full-screen modal showing the document rebuilt from Docling structure
  (headings, paragraphs, lists, tables) + LiteParse character formatting
  (bold/italic/font-size preserved). Demonstrates the value of fusion in one
  glance.
- Requires `docomestria>=0.6.4` which exposes `_merged_document` on the
  terminal step payload.
